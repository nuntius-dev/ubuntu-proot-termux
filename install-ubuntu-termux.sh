...
#######################################################
#  🚀 NUNTIUS DEV ENVIRONMENT - Ultimate Master v3.0
#  
...
step_base() {
    ...
    (apt update -y && apt upgrade -y > /dev/null 2>&1) & spinner $! "Actualizando paquetes..."
    (apt install -y x11-repo tur-repo > /dev/null 2>&1) & spinner $! "Añadiendo repositorios avanzados..."
    (apt install -y proot-distro pulseaudio termux-x11-nightly aria2 wget > /dev/null 2>&1) & spinner $! "Instalando dependencias core..."
}

step_gpu() {
    update_progress
    echo -e "${CYAN}[+] Configurando Aceleración de Hardware (GPU)...${NC}"
    (apt install -y mesa-zink vulkan-loader-android > /dev/null 2>&1) & spinner $! "Instalando backend de Vulkan..."
    
    if [ "$GPU_DRIVER" == "freedreno" ]; then
        (apt install -y mesa-vulkan-icd-freedreno > /dev/null 2>&1) & spinner $! "Instalando drivers Turnip..."
    else
        (apt install -y mesa-vulkan-icd-swrast > /dev/null 2>&1) & spinner $! "Instalando drivers de compatibilidad..."
    fi
...
step_wine() {
    ...
    (apt remove wine-stable -y > /dev/null 2>&1 || true)
    (apt install -y hangover-wine hangover-wowbox64 > /dev/null 2>&1) & spinner $! "Instalando Hangover-Wine y Box64..."
...
step_debian_packages() {
    ...
    (proot-distro login debian -- sh -c "apt update -y && DEBIAN_FRONTEND=noninteractive apt install -y sudo xfce4 xfce4-terminal plank thunar git sassc wget curl dbus-x11 gnupg xdg-utils > /dev/null 2>&1") & spinner $! "Instalando XFCE4 y utilidades..."
...
EOF_THEME
    ...
    (proot-distro login debian -- su - $USERNAME -c "/tmp/theme.sh" > /dev/null 2>&1) & spinner $! "Compilando tema visual..."
}
...
step_chrome() {
    ...
    (proot-distro login debian -- sh -c "wget -q https://dl.google.com/linux/direct/google-chrome-stable_current_arm64.deb -O /tmp/chrome.deb && apt install -y /tmp/chrome.deb > /dev/null 2>&1 && rm -f /tmp/chrome.deb") & spinner $! "Descargando e instalando Chrome..."
    
    # Parche --no-sandbox para entorno Proot
